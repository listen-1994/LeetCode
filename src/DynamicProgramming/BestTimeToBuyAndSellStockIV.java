...
public class BestTimeToBuyAndSellStockIV {
    ...
    public int maxProfit(int k, int[] prices) {
        ...
        dp = new int[prices.length][prices.length][k+1];
        check = new boolean[prices.length][prices.length][k+1];
        core(k, prices, 0, prices.length - 1);
/*        for(int i=0;i<prices.length;i++){
            for(int j=0;j<prices.length;j++){
                System.out.println(i+"|"+j+"|"+"1:"+dp[i][j][1]+":"+check[i][j][1]);
                System.out.println(i+"|"+j+"|"+"2:"+dp[i][j][2]+":"+check[i][j][2]);
            }
        }*/
        return dp[0][prices.length - 1][k];
    }

    public void core(int k, int[] prices, int begin, int end) {
        if(k==1&&!check[begin][end][1]){
            maxProfit(prices,begin,end);
            return;
        }
        int result= 0;
        for(int i=begin+1;i<=end;i++){
            if(!check[begin][i][1]){
                maxProfit(prices,begin,i);
            }
            if(i!=end){
                if(!check[i+1][end][k-1]){
                    core(k-1,prices,i+1,end);
                }
                if(dp[begin][i][1]+dp[i+1][end][k-1]>result){
                    result = dp[begin][i][1]+dp[i+1][end][k-1];
                }
            }else{
                if(dp[begin][end][1]>result){
                    result=dp[begin][end][1];
                }
            }
        }
        dp[begin][end][k]=result;
        check[begin][end][k]=true;
    }

    public void maxProfit(int[] prices,int begin,int end) {
        int minPrice=0;
        int result = 0;
